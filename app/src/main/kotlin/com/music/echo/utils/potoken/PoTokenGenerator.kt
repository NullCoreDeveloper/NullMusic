...
class PoTokenGenerator {
  ...
  fun getWebClientPoToken(videoId: String, sessionId: String): PoTokenResult? {
    ...
    return try {
      Timber.tag(TAG)
        .d("Calling runBlocking to generate poToken (timeout=${POTOKEN_TIMEOUT_MS}ms)...")
      runBlocking {
      ...
      Timber.tag(TAG)
        .w("poToken generation timed out after ${POTOKEN_TIMEOUT_MS}ms; proceeding without PoToken")
      runBlocking {
        webPoTokenGenLock.withLock {
          try {
  ...
  private suspend fun getWebClientPoToken(
    ...
    Timber.tag(TAG)
      .d(
        "poToken generated successfully: session=${streamingPot.take(20)}..., video=${playerPot.take(20)}..."
      )
