 ...
 *
 * Handles both signature deobfuscation (for signatureCipher streams) and n-parameter transformation
 * (for throttle avoidance / 403 fix).
 */
object CipherDeobfuscator {
  ...
  fun initialize(context: Context) {
    ...
    Timber.tag(TAG)
      .d(
        "Known config hashes after init: ${PlayerConfigStore.knownHashes().sorted().joinToString()}"
      )
  ...
  /**
   * The player_ias hash last used to decipher a web stream (sig/n), or null if none yet. Diagnostic
   * only — surfaced in the song-details sheet. Direct-URL clients (ANDROID_VR/IOS) never run the
   * cipher, so this reflects the last web stream.
   */
  ...
  /**
   * SignatureTimestamp of the player JS this cipher actually deciphers with, fetching (or reusing
   * the cached) player JS if needed. API callers must send THIS value in the /player request:
   * during A/B rollouts other sources (e.g. NewPipe's own player fetch) can land on a different
   * player generation, and a sig minted for one player but deciphered by another produces a URL the
   * CDN 403s.
   */
  suspend fun signatureTimestamp(): Int? {
    Timber.tag(TAG).d("Resolving cipher player signatureTimestamp...")
    val (playerJs, hash) =
      PlayerJsFetcher.getPlayerJs(forceRefresh = false)
        ?: run {
  ...
  suspend fun deobfuscateStreamUrl(signatureCipher: String, videoId: String): String? =
    deobfuscateMutex.withLock {
      try {
        deobfuscateInternal(signatureCipher, videoId, isRetry = false)?.also {
        ...
        try {
          ...
          closeWebView()
          deobfuscateInternal(signatureCipher, videoId, isRetry = true)?.also {
            rendererRecoveryPolicy.onSuccess()
          }
   ...
   * legacy-regex false positive — is invisible to [deobfuscateStreamUrl]'s exception-retry, so the
   * rejected stream is the only signal it was wrong. Re-fetch the player-config table
  ...
  private suspend fun deobfuscateInternal(
    ...
    Timber.tag(TAG)
      .d("  s (obfuscated sig): ${obfuscatedSig?.take(30)}... (length=${obfuscatedSig?.length})")
    ...
    if (obfuscatedSig == null || baseUrl == null) {
      Timber.tag(TAG)
        .e(
          "Could not parse signatureCipher params: s=${obfuscatedSig != null}, url=${baseUrl != null}"
        )
      return null
    }
    ...
    Timber.tag(TAG)
      .d(
        "Deobfuscated signature: ${deobfuscatedSig.take(30)}... (length=${deobfuscatedSig.length})"
      )
   ...
   *
   * Uses the runtime-discovered n-function from the player JS WebView. Returns the URL with the
   * transformed 'n' value, or the original URL if transform fails.
   *
   * IMPORTANT: This must be called for WEB_REMIX, WEB, WEB_CREATOR, TVHTML5 clients and for
   * privately owned tracks (uploaded songs).
   */
  ...
  private suspend fun transformNInternal(url: String): String {
    ...
    val transformedUrl = url.replaceFirst(Regex("([?&])n=[^&]+"), "$1n=${Uri.encode(transformedN)}")
  ...
  /**
   * A renderer death (or renderer-gone-equivalent timeout) was detected: drop the dead instance so
   * the next call recreates it, and record the failure so repeated deaths open the
   * [rendererRecoveryPolicy] backoff window. Must be called with [deobfuscateMutex] held.
   */
  ...
  private suspend fun getOrCreateWebView(forceRefresh: Boolean): CipherWebView? {
    Timber.tag(TAG)
      .d("getOrCreateWebView: forceRefresh=$forceRefresh, existing=${cipherWebView != null}")
    ...
    if (!sigFromConfig || !nFromConfig) {
      Timber.tag(TAG)
        .w(
          "Extraction not fully config-backed for player $hash (sigConfig=$sigFromConfig, nConfig=$nFromConfig; sig=${analysis.sigInfo != null}, n=${analysis.nFuncInfo != null}) — forcing remote config refresh"
        )
      ...
      if (healed) {
        ...
        Timber.tag(TAG)
          .d(
            "Re-extracted after refresh: sigConfig=${analysis.sigInfo?.isHardcoded == true}, nConfig=${analysis.nFuncInfo?.isHardcoded == true}"
          )
      }
    }
    ...
    Timber.tag(TAG)
      .d(
        "  sig: ${analysis.sigInfo.name} (constantArg=${analysis.sigInfo.constantArg}, hardcoded=${analysis.sigInfo.isHardcoded})"
      )
    Timber.tag(TAG)
      .d(
        "  nFunc: ${analysis.nFuncInfo?.name}[${analysis.nFuncInfo?.arrayIndex}] (hardcoded=${analysis.nFuncInfo?.isHardcoded})"
      )
