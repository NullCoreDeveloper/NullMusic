...
object Fix403 {
   ...
   */
  @JvmField val ENABLED: Boolean = BuildConfig.DEBUG
  ...
  /**
   * Log a throwable with its full cause chain **and** stack trace. Use this everywhere an exception
   * is caught — including places that intentionally continue, because a swallowed exception is
   * exactly what made the earlier reports impossible to diagnose.
   */
  ...
  /**
   * Run [block], logging any throwable in full, and return `null` on failure. For call sites that
   * legitimately continue after an error but must not hide it.
   */
