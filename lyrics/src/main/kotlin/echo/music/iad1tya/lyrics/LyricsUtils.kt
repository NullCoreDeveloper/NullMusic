...
object LyricsUtils {
  ...
  private val RUSSIAN_CYRILLIC_LETTERS =
    setOf(
      "А",
      "Б",
      "В",
  ...
  private val UKRAINIAN_CYRILLIC_LETTERS =
    setOf(
      "А",
      "Б",
      "В",
  ...
  private val SERBIAN_CYRILLIC_LETTERS =
    setOf(
      "А",
      "Б",
      "В",
  ...
  private val BULGARIAN_CYRILLIC_LETTERS =
    setOf(
      "А",
      "Б",
      "В",
  ...
  private val BELARUSIAN_CYRILLIC_LETTERS =
    setOf(
      "А",
      "Б",
      "В",
  ...
  private val KYRGYZ_CYRILLIC_LETTERS =
    setOf(
      "А",
      "Б",
      "В",
  ...
  private val MACEDONIAN_CYRILLIC_LETTERS =
    setOf(
      "А",
      "Б",
      "В",
  ...
  private val SERBIAN_SPECIFIC_CYRILLIC_LETTERS =
    setOf("Ђ", "ђ", "Ј", "ј", "Љ", "љ", "Њ", "њ", "Ћ", "ћ", "Џ", "џ")
  ...
  fun parseLyrics(lyrics: String): List<LyricsEntry> {
    ...
    val lines =
      decodedLyrics.lines().filter { it.isNotBlank() && !it.trim().startsWith("[offset:") }
  ...
  private fun parseRichSyncLyrics(lines: List<String>): List<LyricsEntry> {
    ...
    lines.forEachIndexed { index, line ->
      ...
      if (matchResult != null) {
        ...
        val millisPart =
          if (matchResult.groupValues[3].length == 3) centiseconds else centiseconds * 10
        val lineTimeMs =
          minutes * DateUtils.MINUTE_IN_MILLIS + seconds * DateUtils.SECOND_IN_MILLIS + millisPart
  ...
  private fun parseRichSyncWords(
    ...
    wordMatches.forEachIndexed { index, match ->
      ...
      val fractionPart =
        if (match.groupValues[3].length == 3) fraction / 1000.0 else fraction / 100.0
      ...
      val endTimeSeconds =
        if (index < wordMatches.size - 1) {
          ...
          val nextFractionPart =
            if (nextMatch.groupValues[3].length == 3) nextFraction / 1000.0
            else nextFraction / 100.0
  ...
  private fun getNextLineStartTime(currentIndex: Int, allLines: List<String>): Double? {
    ...
    val fractionPart =
      if (matchResult.groupValues[3].length == 3) fraction / 1000.0 else fraction / 100.0
  ...
  suspend fun romanizeCyrillic(text: String): String? =
    withContext(Dispatchers.Default) {
  ...
  private fun romanizeRussianInternal(text: String): String {
    ...
    val words =
      text.split("((?<=\\s|[.,!?;])|(?=\\s|[.,!?;]))".toRegex()).filter { it.isNotEmpty() }
      ...
      if (word.matches("[.,!?;]".toRegex()) || word.isBlank()) {
        ...
        while (charIndex < word.length) {
          ...
          if (!consumed) {
            ...
            if (
              (charStr == "е" || charStr == "Е") &&
                (charIndex == 0 || word[charIndex - 1].isWhitespace())
            ) {
    ...
    words.forEachIndexed { _, word ->
      if (word.matches("[.,!?;]".toRegex()) || word.isBlank()) {
        ...
        while (charIndex < word.length) {
          ...
          if (
            charIndex > 0 && word[charIndex - 1].isLetter() && !isCyrillicVowel(word[charIndex - 1])
          ) {
          ...
          if (!processed) {
            romajiBuilder.append(
              UKRAINIAN_ROMAJI_MAP[charStr] ?: GENERAL_CYRILLIC_ROMAJI_MAP[charStr] ?: charStr
            )
          }
          charIndex++
        }
    ...
    words.forEachIndexed { _, word ->
      if (word.matches("[.,!?;]".toRegex()) || word.isBlank()) {
        ...
        while (charIndex < word.length) {
          val charStr = word[charIndex].toString()
          val romanizedChar =
            SERBIAN_ROMAJI_MAP[charStr] ?: GENERAL_CYRILLIC_ROMAJI_MAP[charStr] ?: charStr
          romajiBuilder.append(romanizedChar)
          charIndex++
        }
      }
    ...
    words.forEachIndexed { _, word ->
      if (word.matches("[.,!?;]".toRegex()) || word.isBlank()) {
        ...
        while (charIndex < word.length) {
          val charStr = word[charIndex].toString()
          val romanizedChar =
            BULGARIAN_ROMAJI_MAP[charStr] ?: GENERAL_CYRILLIC_ROMAJI_MAP[charStr] ?: charStr
          romajiBuilder.append(romanizedChar)
          charIndex++
        }
      }
      ...
      if (word.matches("[.,!?;]".toRegex()) || word.isBlank()) {
        ...
        while (charIndex < word.length) {
          ...
          if (
            ...
            val romanizedChar =
              BELARUSIAN_ROMAJI_MAP[charStr] ?: GENERAL_CYRILLIC_ROMAJI_MAP[charStr] ?: charStr
            romajiBuilder.append(romanizedChar)
          }
          charIndex += 1
        }
    ...
    words.forEachIndexed { _, word ->
      if (word.matches("[.,!?;]".toRegex()) || word.isBlank()) {
        ...
        while (charIndex < word.length) {
          val charStr = word[charIndex].toString()
          val romanizedChar =
            KYRGYZ_ROMAJI_MAP[charStr] ?: GENERAL_CYRILLIC_ROMAJI_MAP[charStr] ?: charStr
          romajiBuilder.append(romanizedChar)
          charIndex++
        }
      }
    ...
    words.forEachIndexed { _, word ->
      if (word.matches("[.,!?;]".toRegex()) || word.isBlank()) {
        ...
        while (charIndex < word.length) {
          val charStr = word[charIndex].toString()
          val romanizedChar =
            MACEDONIAN_ROMAJI_MAP[charStr] ?: GENERAL_CYRILLIC_ROMAJI_MAP[charStr] ?: charStr
          romajiBuilder.append(romanizedChar)
          charIndex++
        }
      }
  ...
  fun isRussian(text: String): Boolean {
    return text.any { char -> RUSSIAN_CYRILLIC_LETTERS.contains(char.toString()) } &&
      text.all { char ->
        ...
        RUSSIAN_CYRILLIC_LETTERS.contains(charStr) ||
          !charStr.matches("[\\u0400-\\u04FF]".toRegex())
      }
  }

  fun isUkrainian(text: String): Boolean {
    return text.any { char ->
      UKRAINIAN_CYRILLIC_LETTERS.contains(char.toString()) ||
        UKRAINIAN_SPECIFIC_CYRILLIC_LETTERS.contains(char.toString())
    } &&
      text.all { char ->
        UKRAINIAN_CYRILLIC_LETTERS.contains(char.toString()) ||
          UKRAINIAN_SPECIFIC_CYRILLIC_LETTERS.contains(char.toString()) ||
  ...
  fun isSerbian(text: String): Boolean {
    return text.any { char ->
      SERBIAN_CYRILLIC_LETTERS.contains(char.toString()) ||
        SERBIAN_SPECIFIC_CYRILLIC_LETTERS.contains(char.toString())
    } &&
      text.all { char ->
        SERBIAN_CYRILLIC_LETTERS.contains(char.toString()) ||
          SERBIAN_SPECIFIC_CYRILLIC_LETTERS.contains(char.toString()) ||
  ...
  fun isBelarusian(text: String): Boolean {
    return text.any { char ->
      BELARUSIAN_CYRILLIC_LETTERS.contains(char.toString()) ||
        BELARUSIAN_SPECIFIC_CYRILLIC_LETTERS.contains(char.toString())
    } &&
      text.all { char ->
        BELARUSIAN_CYRILLIC_LETTERS.contains(char.toString()) ||
          BELARUSIAN_SPECIFIC_CYRILLIC_LETTERS.contains(char.toString()) ||
  ...
  fun isKyrgyz(text: String): Boolean {
    return text.any { char ->
      KYRGYZ_CYRILLIC_LETTERS.contains(char.toString()) ||
        KYRGYZ_SPECIFIC_CYRILLIC_LETTERS.contains(char.toString())
    } &&
      text.all { char ->
        KYRGYZ_CYRILLIC_LETTERS.contains(char.toString()) ||
          KYRGYZ_SPECIFIC_CYRILLIC_LETTERS.contains(char.toString()) ||
  ...
  fun isMacedonian(text: String): Boolean {
    return text.any { char ->
      MACEDONIAN_CYRILLIC_LETTERS.contains(char.toString()) ||
        MACEDONIAN_SPECIFIC_CYRILLIC_LETTERS.contains(char.toString())
    } &&
      text.all { char ->
        MACEDONIAN_CYRILLIC_LETTERS.contains(char.toString()) ||
          MACEDONIAN_SPECIFIC_CYRILLIC_LETTERS.contains(char.toString()) ||
