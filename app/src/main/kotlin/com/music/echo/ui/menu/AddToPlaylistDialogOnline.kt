...
/**
 * Ranks a full collaboration match (every target artist present, and only target artists) above a
 * partial one, and requires exact (not substring) name equality throughout — a `contains` check
 * would let a short or generic target name ("Nas") false-positive against an unrelated artist
 * ("Nasty"), and would let a single-artist partial match win over the correct multi-artist
 * collaboration purely because it happened to come first.
 */
private fun findBestMatchingSong(items: List<SongItem>, targetArtists: List<String>): SongItem? {
  val normalizedTargets =
    targetArtists.map { it.trim().lowercase() }.filter { it.isNotEmpty() }.toSet()
...
fun AddToPlaylistDialogOnline(
  ...
  val (sortType, onSortTypeChange) =
    rememberEnumPreference(AddToPlaylistSortTypeKey, PlaylistSortType.NAME)
  val (sortDescending, onSortDescendingChange) =
    rememberPreference(AddToPlaylistSortDescendingKey, false)
  ...
  val songIds by remember { mutableStateOf<List<String>?>(null) }
  val duplicates by remember { mutableStateOf(emptyList<String>()) }
                      ...
                      YouTube.search(query, YouTube.SearchFilter.FILTER_SONG)
                        .onSuccess { result ->
                          ...
                          if (items.isNotEmpty()) {
                            ...
                            val songItems =
                              itemsPage?.items?.filterIsInstance<SongItem>() ?: emptyList()
                          ...
                          if (items.isNotEmpty()) {
                            ...
                            if (firstSong != null) {
                              ...
                              withContext(Dispatchers.IO) {
                                try {
                                  database.insert(firstSongMedia)
                                  database.query { update(firstSongEnt.toggleLike()) }
                                } catch (e: Exception) {
                                  Timber.tag("Exception inserting song in database:")
                                    .e(e.toString())
                                }
        ...
        TextButton(
          onClick = {
            ...
            database.transaction {
              addSongToPlaylist(selectedPlaylist!!, songIds!!.filter { !duplicates.contains(it) })
            }
            Toast.makeText(
                context,
