...
fun EqScreen(
        ...
        try {
          ...
          if (inputStream != null) {
            viewModel.importCustomProfile(
              ...
              onSuccess = { Timber.d("Custom EQ profile imported successfully: $fileName") },
              onError = { error ->
      ...
      playerConnection?.let { connection ->
        val intent =
          Intent(AudioEffect.ACTION_DISPLAY_AUDIO_EFFECT_CONTROL_PANEL).apply {
            putExtra(AudioEffect.EXTRA_AUDIO_SESSION, connection.player.audioSessionId)
  ...
  if (showError != null) {
    AlertDialog(
      ...
      text = { Text(showError ?: "") },
      confirmButton = {
        TextButton(onClick = { showError = null }) { Text(stringResource(android.R.string.ok)) }
      }
    )
  }
          ...
          item {
            Box(
              ...
              Column(horizontalAlignment = Alignment.CenterHorizontally) {
                ...
                Button(onClick = onImportCustomEQ) { Text(stringResource(R.string.import_profile)) }
                Spacer(modifier = Modifier.height(8.dp))
