 ...
 *
 * Handles both legacy patterns and modern Q-array obfuscation (2025+). Falls back to hardcoded
 * configs for known player.js hashes when regex fails.
 */
object FunctionNameExtractor {
  ...
  /**
   * Hardcoded player.js configuration for when regex extraction fails Due to Q-array obfuscation,
   * patterns like `.get("n")` become `Q[T^6001]`
   */
  ...
  /**
   * Get the validated config for a known player.js hash from [PlayerConfigStore] (the bundled asset
   * overlaid by the remote table). Replaces the former hardcoded map — new players are now added by
   * pushing to player_configs.json, not by editing this file.
   */
   ...
   *
   * Validated config FIRST, legacy regex heuristics only as a fallback: config entries are proven
   * against the live CDN (HTTP 206) before they ship, while the patterns below are unanchored
   * heuristics that can false-match anywhere in the ~2 MB player JS. A heuristic must never shadow
   * a validated config — and a false positive here must not block the unknown-player forced refresh
  ...
  fun extractSigFunctionInfo(playerJs: String, knownHash: String? = null): SigFunctionInfo? {
    ...
    if (hashToUse != null) {
      ...
      if (config != null) {
        if (config.sigJsExpression != null) {
          ...
          Timber.tag(TAG)
            .d(
              "USING HARDCODED SIG FUNCTION: ${config.sigFuncName}(${config.sigConstantArgs}, ...)"
            )
          Timber.tag(TAG)
            .d("Sig preprocess: ${config.sigPreprocessFunc}(${config.sigPreprocessArgs}, sig)")
        }
    ...
    for ((index, pattern) in SIG_FUNCTION_PATTERNS.withIndex()) {
      ...
      if (match != null) {
        ...
        Timber.tag(TAG)
          .d(
            "  match context: ...${playerJs.substring(maxOf(0, match.range.first - 20), minOf(playerJs.length, match.range.last + 20))}..."
          )
   ...
   *
   * Validated config FIRST, legacy regex heuristics only as a fallback — same precedence rationale
   * as [extractSigFunctionInfo].
   *
   * @param playerJs The player.js content
   * @param knownHash Optional hash for config lookup
   */
    ...
    if (hashToUse != null) {
      ...
      if (config != null) {
        if (config.nJsExpression != null) {
          ...
          Timber.tag(TAG)
            .d("USING HARDCODED N-FUNCTION: ${config.nFuncName}[${config.nArrayIndex}]")
  ...
  fun analyzePlayerJs(playerJs: String, knownHash: String? = null): PlayerAnalysis {
    ...
    Timber.tag(TAG)
      .d("Sig Function:       ${sigInfo?.name ?: "NOT FOUND"} (hardcoded=${sigInfo?.isHardcoded})")
    Timber.tag(TAG).d("Sig Constant Arg:   ${sigInfo?.constantArg}")
    Timber.tag(TAG)
      .d(
        "N-Function:         ${nFuncInfo?.name ?: "NOT FOUND"} (hardcoded=${nFuncInfo?.isHardcoded})"
      )
