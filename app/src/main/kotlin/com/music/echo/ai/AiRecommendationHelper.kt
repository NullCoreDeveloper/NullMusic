  ...
  suspend fun generateRecommendations(context: Context, onLog: (suspend (String) -> Unit)? = null) =
    withContext(Dispatchers.IO) {
      ...
      val likedSongs: List<Song> =
        database.likedSongsByPlayTimeAsc().firstOrNull()?.take(20) ?: emptyList()
      ...
      var playlist =
        database
          .searchPlaylists(PLAYLIST_NAME)
          .firstOrNull()
          ?.find { it.playlist.name == PLAYLIST_NAME }
          ?.playlist
      if (playlist == null) {
        playlist =
          PlaylistEntity(
      ...
      resolvedSongs.forEachIndexed { index, songItem ->
        ...
        database.insert(
          PlaylistSongMap(playlistId = playlist.id, songId = songItem.id, position = index)
        )
      }
