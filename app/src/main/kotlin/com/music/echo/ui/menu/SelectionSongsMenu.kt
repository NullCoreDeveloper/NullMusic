...
fun SelectionSongMenu(
  ...
  val allInLibrary by remember {
    mutableStateOf(
      songSelection.all { it.song.inLibrary != null },
    )
  }
  ...
  val allLiked by
    remember(songSelection) {
      mutableStateOf(
        songSelection.isNotEmpty() && songSelection.all { it.song.liked },
      )
    }
          ...
          buildList {
            ...
            add(
              Material3MenuItemData(
                ...
                onClick = {
                  if (allInLibrary) {
                    database.query { songSelection.forEach { song -> inLibrary(song.id, null) } }
                    coroutineScope.launch {
                      songSelection.forEach { song -> YouTube.toggleSongLibrary(song.id, false) }
                    }
                  } else {
                    ...
                    coroutineScope.launch {
                      songSelection
                        ...
                        .forEach { song -> YouTube.toggleSongLibrary(song.id, true) }
                    }
                  }
                }
              )
          ...
          buildList {
            ...
            add(
              Material3MenuItemData(
                title = {
                  Text(
                    text = stringResource(if (allLiked) R.string.dislike_all else R.string.like_all)
                  )
                },
                icon = {
                  Icon(
...
fun SelectionMediaMetadataMenu(
  ...
  AddToPlaylistDialog(
    ...
    onGetSong = {
      songSelection.map {
        runBlocking { withContext(Dispatchers.IO) { database.insert(it) } }
        it.id
      }
    },
    onDismiss = { showChoosePlaylistDialog = false }
                ...
                Material3MenuItemData(
                  ...
                  onClick = {
                    ...
                    currentItems.forEach { cur ->
                      if (
                        playerConnection.player.availableCommands.contains(
      ...
      Material3MenuGroup(
        items =
          buildList {
            add(
              Material3MenuItemData(
                title = { Text(text = stringResource(R.string.like_all)) },
                icon = {
                  Icon(
                    painter =
                      painterResource(
                ...
                onClick = {
                  database.query {
                    if (allLiked) {
                      songSelection.forEach { song -> update(song.toSongEntity().toggleLike()) }
                    } else {
                      songSelection
                        .filter { !it.liked }
                        .forEach { song -> update(song.toSongEntity().toggleLike()) }
                    }
                  }
                }
              )
