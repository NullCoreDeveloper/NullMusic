...
private constructor(
  ...
  /**
   * Single-shot continuation slot with an id-checked take. arm() returns the request id the JS call
   * must echo back; takeIfCurrent(id) ignores late callbacks from superseded requests (the
   * stale-result guard); takeAny() is for renderer-gone/timeout paths, which must clear whatever is
   * pending. Synchronized because JS-bridge callbacks arrive on a WebView-internal thread while
   * onRenderProcessGone/timeouts run on the main thread. The distinct method names are deliberate:
   * same-name overloads made dropping the id a silent, compile-clean way to reintroduce the race.
   */
  private class RequestSlot<T> {
    ...
    @Synchronized fun takeAny(): Continuation<T>? = continuation.also { continuation = null }
  }

  /**
   * Set once the WebView's renderer process has died (or an evaluate timed out, which on a wedged
   * renderer is indistinguishable). A dead instance must be discarded and recreated — per Android
   * docs a WebView whose render process is gone cannot be reused.
   */
  @Volatile
  ...
  init {
    ...
    Timber.tag(TAG)
      .d(
        "  sigInfo: name=${sigInfo?.name}, constantArg=${sigInfo?.constantArg}, hardcoded=${sigInfo?.isHardcoded}"
      )
    Timber.tag(TAG)
      .d(
        "  nFuncInfo: name=${nFuncInfo?.name}, arrayIdx=${nFuncInfo?.arrayIndex}, hardcoded=${nFuncInfo?.isHardcoded}"
      )
    ...
    webView.webViewClient =
      object : WebViewClient() {
        ...
        override fun onRenderProcessGone(view: WebView, detail: RenderProcessGoneDetail): Boolean {
          ...
          onRendererGone(
            "WebView render process gone (didCrash=${runCatching { detail.didCrash() }.getOrNull()})"
          )
  ...
  /**
   * The renderer died (or is treated as dead after a timeout): fail every pending continuation with
   * [CipherRendererGoneException] so create()/decipher fail fast instead of hanging forever on
   * JS-bridge callbacks that will never come (and so CipherDeobfuscator's mutex is released), then
   * destroy the WebView — it cannot be reused after a render-process crash.
   */
   ...
   * [buildModifiedPlayerJsImpl]) into the WebView. Only the cheap WebView work happens here on
   * Main.
   */
...
<script src="player.js"
  ...
  suspend fun deobfuscateSignature(obfuscatedSig: String): String {
    ...
    return try {
      withTimeout(EVAL_TIMEOUT_MS) {
        withContext(Dispatchers.Main) {
          suspendCancellableCoroutine { cont ->
            ...
            val jsCall =
              "deobfuscateSig('${sigInfo.name}', $constArgJs, '${escapeJsString(obfuscatedSig)}', $requestId)"
      ...
      Timber.tag(TAG)
        .e("Sig deobfuscation timed out after ${EVAL_TIMEOUT_MS}ms — treating renderer as gone")
  ...
  suspend fun transformN(nValue: String): String {
    ...
    return try {
      ...
      Timber.tag(TAG)
        .e("N-transform timed out after ${EVAL_TIMEOUT_MS}ms — treating renderer as gone")
  ...
  private fun throwIfDead() {
    if (isDead) {
      throw CipherRendererGoneException(
        "CipherWebView renderer is gone — instance must be recreated"
      )
    }
  }
  ...
  companion object {
    ...
    private fun buildModifiedPlayerJsImpl(
      ...
      val exports = buildList {
        ...
        if (sigJsExpr != null) {
          ...
          add(
            "window._cipherSigFunc = function(sig) { try { return $expr; } catch(e) { return null; } };"
          )
          ...
          if (
            !sigConstArgs.isNullOrEmpty() &&
              preprocessFunc != null &&
              !preprocessArgs.isNullOrEmpty()
          ) {
            ...
            add(
              "window._cipherSigFunc = function(sig) { return $sigFuncName($mainArgsStr, $preprocessFunc($prepArgsStr, sig)); };"
            )
        ...
        if (nJsExpr != null) {
          ...
          add(
            "window._nTransformFunc = function(n) { try { return $expr; } catch(e) { return n; } };"
          )
    ...
    suspend fun create(
      ...
      val cacheDir =
        withContext(Dispatchers.IO) {
          ...
          Timber.tag(TAG)
            .d(
              "Player.js written to cache: ${playerJsFile.absolutePath} (${modifiedJs.length} chars)"
            )
          dir
        }
      ...
      try {
        ...
        Timber.tag(TAG)
          .e(
            "CipherWebView init timed out after ${CREATE_TIMEOUT_MS}ms — treating renderer as gone"
          )
        destroyQuietly(created)
        throw CipherRendererGoneException(
          "CipherWebView init timed out after ${CREATE_TIMEOUT_MS}ms"
        )
      } catch (e: Exception) {
    ...
    companion object {
        private const val TAG = "nullmusic_CipherWebView"
        private const val JS_INTERFACE = "CipherBridge"
...
/**
 * The cipher WebView's render process died (kernel OOM kill, crash) or stopped responding. The
 * instance is unusable; callers must drop it and decide whether recreating is worth it (see
