...
fun CircularEqControl(
  ...
  Canvas(
      ...
      modifier.pointerInput(enabled) {
        ...
        awaitEachGesture {
          ...
          while (true) {
            ...
            val dragV =
              calcValueForAxis(change.position, size.width.toFloat(), size.height.toFloat(), axis)
    ...
    for (i in 0..2) {
      ...
      drawCircle(
        primary.copy(alpha = if (isActive) 0.2f else 0.1f),
        if (isActive) 32f else 26f,
        points[i]
      )
    ...
    for (i in 0..2) {
      ...
      drawText(
        ...
        Offset(lx - label.size.width / 2f, ly - label.size.height - 2f)
      )
