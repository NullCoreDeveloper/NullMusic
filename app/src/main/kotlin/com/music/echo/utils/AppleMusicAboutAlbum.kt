...
object AppleMusicAboutAlbum {
  ...
  suspend fun fetchAlbumDescription(
    ...
    return runCatching {
        val query =
          if (artistName != null && !albumTitle.contains(artistName, ignoreCase = true)) {
        ...
        val albumsData =
          searchRoot["results"]?.jsonObject?.get("albums")?.jsonObject?.get("data")?.jsonArray
            ...
            .mapNotNull { item ->
              ...
              if (artistName != null) {
                if (resultArtistName.equals(artistName, ignoreCase = true)) score += 10
                  ...
                  resultArtistName.contains(artistName, ignoreCase = true) ||
                    artistName.contains(resultArtistName, ignoreCase = true)
                )
                  score += 5
              }
              ...
              if (resultName.equals(albumTitle, ignoreCase = true)) score += 10
                ...
                resultName.contains(albumTitle, ignoreCase = true) ||
                  albumTitle.contains(resultName, ignoreCase = true)
              )
                score += 5
