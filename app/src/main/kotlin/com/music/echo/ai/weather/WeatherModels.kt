...
fun mapWmoCodeToCondition(code: Int, isDay: Boolean): Pair<String, String> {
  return when (code) {
    ...
    55 -> "Drizzle" to "🌦️"
    56,
    57 -> "Freezing Drizzle" to "🌧️"
    61 -> "Light Rain" to "🌧️"
    63 -> "Moderate Rain" to "🌧️"
    65 -> "Heavy Rain" to "🌧️"
    66,
    67 -> "Freezing Rain" to "🌧️"
    71 -> "Light Snow" to "🌨️"
    ...
    80,
    81,
    82 -> "Rain Showers" to "🌦️"
    85,
    86 -> "Snow Showers" to "🌨️"
    95 -> "Thunderstorm" to "⛈️"
    96,
    99 -> "Heavy Thunderstorm" to "⛈️"
...
fun parseWeatherSnapshot(params: String?): WeatherInfo? {
  ...
  return try {
    ...
    if (parts.size >= 4) {
      ...
      val temp =
        parts.getOrNull(4)?.toDoubleOrNull()
          ?: parts[0].substringBefore("°").toDoubleOrNull()
          ?: 20.0
      WeatherInfo(
        temperature = temp,
        feelsLike = temp,
