 ...
 *
 * Uses the same AST-based approach as yt-dlp's EJS solver (meriyah + astring + yt.solver.core.js)
 * to reliably extract and execute the n-transform function from the YouTube player JS.
 */
object EjsNTransformSolver {
  ...
  /**
   * Transform the 'n' parameter in a SABR streaming URL. Returns the URL with the transformed 'n'
   * value, or the original URL if transform fails.
   */
    ...
    private fun loadSolverAndPlayer() {
      ...
      for (file in listOf("meriyah.js", "astring.js", "yt.solver.core.js")) {
        assetManager.open("solver/$file").use { input ->
          File(cacheDir, file).outputStream().use { output -> input.copyTo(output) }
        }
      }
