...
class SponsorBlockRepository @Inject constructor() {
  ...
  suspend fun getSkipSegments(videoId: String): List<SponsorBlockSegment> =
    withContext(Dispatchers.IO) {
      ...
      try {
        val categories =
          listOf("sponsor", "intro", "outro", "interaction", "selfpromo", "music_offtopic")
        val categoriesParam =
          categories.joinToString(separator = "%22,%22", prefix = "[%22", postfix = "%22]")
        val url =
        ...
        for (i in 0 until jsonArray.length()) {
          ...
          if (segmentArray.length() == 2) {
            segments.add(
              SponsorBlockSegment(
                segment =
                  listOf(segmentArray.getDouble(0).toFloat(), segmentArray.getDouble(1).toFloat()),
