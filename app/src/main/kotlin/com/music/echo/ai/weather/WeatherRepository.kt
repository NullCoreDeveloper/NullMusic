...
object WeatherRepository {
  ...
  suspend fun fetchWeather(latitude: Double, longitude: Double): Result<WeatherInfo> =
    withContext(Dispatchers.IO) {
      val url =
        "https://api.open-meteo.com/v1/forecast?latitude=$latitude&longitude=$longitude&current=temperature_2m,relative_humidity_2m,apparent_temperature,is_day,weather_code,wind_speed_10m"
        ...
        client.newCall(request).execute().use { response ->
          ...
          val current =
            json.optJSONObject("current")
              ?: return@withContext Result.failure(Exception("Invalid weather data"))
