...
object LocationProvider {
    ...
    withContext(Dispatchers.IO) {
      try {
        ...
        for (provider in providers) {
          ...
          if (isEnabled) {
            ...
            if (loc != null && isLocationFresh(loc)) {
              if (
                bestLocation == null || loc.elapsedRealtimeNanos > bestLocation.elapsedRealtimeNanos
              ) {
                bestLocation = loc
              }
            }
        ...
        try {
          ...
          val getFusedMethod =
            fusedClientClass.getMethod("getFusedLocationProviderClient", Context::class.java)
  ...
  private suspend fun <T> suspendTaskResult(task: Any?): T? =
    withContext(Dispatchers.IO) {
