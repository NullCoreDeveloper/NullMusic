...
class MusicRecognizerWidgetReceiver : AppWidgetProvider() {
  ...
  private fun handleStartRecognition(context: Context) {
    ...
    if (
      currentState == STATE_SUCCESS || currentState == STATE_NO_MATCH || currentState == STATE_ERROR
    ) {
  ...
  private fun updateAllWidgets(context: Context, appWidgetManager: AppWidgetManager) {
    ...
    val albumArtBitmap =
      if (state == STATE_SUCCESS && coverArtPath.isNotEmpty()) {
        try {
  ...
  private fun createWideViews(
    ...
    applyMicState(
      ...
      state,
      pulseFrame,
      R.id.widget_recognizer_mic_container,
      R.id.widget_recognizer_pulse
    )
  ...
  private fun createTinyViews(context: Context, state: Int, pulseFrame: Int): RemoteViews {
    val views = RemoteViews(context.packageName, R.layout.widget_recognizer_tiny)
    applyMicState(
      views,
      state,
      pulseFrame,
      R.id.widget_recognizer_tiny_mic_container,
      R.id.widget_recognizer_tiny_pulse
    )
  ...
  private fun applyAlbumArt(views: RemoteViews, state: Int, albumArt: android.graphics.Bitmap?) {
    ...
    when (state) {
      STATE_IDLE -> {
        views.setTextViewText(
          R.id.widget_recognizer_song_title,
  ...
  /**
   * Tap on song info area / album art → always open the recognition screen. On SUCCESS the screen
   * will show the result that the widget service already set on
