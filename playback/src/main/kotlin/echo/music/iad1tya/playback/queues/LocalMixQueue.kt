...
class LocalMixQueue(
  ...
  override suspend fun getInitialStatus(): Queue.Status =
    withContext(Dispatchers.IO) {
      ...
      val relatedSongs = playlistSongIds.flatMap { songId -> database.relatedSongs(songId) }
      val uniqueRelated =
        relatedSongs.filter { song -> song.id !in playlistSongIds }.distinctBy { it.id }
      ...
      val onlineMix = (fromNext + fromRelated).distinctBy { it.mediaId }.take(maxMixSize)
