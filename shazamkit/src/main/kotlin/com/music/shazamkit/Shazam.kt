...
object Shazam {
  ...
  private suspend fun enqueueRequest(
    ...
    val request =
      requestMutex.withLock {
        ...
        val request =
          PendingRequest(id = requestId, signature = signature, sampleDurationMs = sampleDurationMs)
  ...
  private fun cacheResult(key: String, result: RecognitionResult) {
    resultCache[key] = CachedResult(timestamp = System.currentTimeMillis(), result = result)
  ...
  private fun ShazamResponseJson.toRecognitionResult(): RecognitionResult? {
    ...
    val spotifyProvider =
      track.hub?.providers?.find { it?.caption?.contains("spotify", ignoreCase = true) == true }
