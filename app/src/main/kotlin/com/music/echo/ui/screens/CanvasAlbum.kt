...
fun rememberAlbumCanvas(
  ...
  val cacheKey =
    remember(albumTitle, artistName) {
      if (albumTitle != null && artistName != null) "album|$albumTitle|$artistName" else null
    }

  var canvasArtwork by
    remember(cacheKey) { mutableStateOf(cacheKey?.let { CanvasArtworkPlaybackCache.get(it) }) }
    ...
    val fetched =
        ...
        searchTasks
          ...
          .firstNotNullOfOrNull { (s, a) ->
            nullmusicCanvasProvider.getBySongArtist(song = s, artist = a)?.takeIf {
              ...
              ?: TidalCanvasProvider.getByAlbumArtist(album = s, artist = a)?.takeIf {
                !it.preferredAnimationUrl.isNullOrBlank()
              }
          }
      }
      ...
      validated =
        tidalFetched?.let { artwork ->
          ...
          val albumMatches =
            if (canvasAlbumName != null && !albumTitle.isNullOrBlank()) {
