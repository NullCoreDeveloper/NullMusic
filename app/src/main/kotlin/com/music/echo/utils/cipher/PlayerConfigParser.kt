 ...
 *
 * Pure JVM on purpose: no Android or Timber imports, so the full validation surface is coverable by
 * plain unit tests. Android concerns (assets, cache files, fetching) live in [PlayerConfigStore].
 *
 * Security boundary: every value in this file ends up evaluated as JavaScript inside the cipher
 * WebView, so entries are regex-locked to shapes that cannot carry arbitrary JS — `sig` must be a
 * single `name(int,int,INPUT)` call and `nClass` a bare identifier; the n-transform IIFE is built
 * locally from [buildNJsExpression], never taken from the file.
 */
object PlayerConfigParser {
  ...
  /**
   * Parses [jsonText]. File-level problems (malformed JSON, missing/unsupported schemaVersion,
   * missing `players`) return [ParseResult.Failure] — callers keep their previous map. Invalid
   * individual entries are skipped and reported in [ParseResult.Success.skippedEntries] so one bad
   * entry can't poison the rest.
   */
  fun parse(jsonText: String): ParseResult {
    ...
    val schemaVersion =
      (root["schemaVersion"] as? JsonPrimitive)?.takeIf { !it.isString }?.content?.toIntOrNull()
    ...
    if (schemaVersion > SUPPORTED_SCHEMA_VERSION) {
      return ParseResult.Failure(
        "unsupported schemaVersion $schemaVersion (supported: $SUPPORTED_SCHEMA_VERSION)"
      )
    }
  ...
  private fun parseEntry(
    ...
    val config =
      FunctionNameExtractor.HardcodedPlayerConfig(
